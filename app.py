...
with col_input:
    # Enter'a basıldığında tetiklenme mekanizması eklendi
    tahmin_input = st.text_input("Tahmin", placeholder="Guess", key="user_guess")

with col_btn:
    btn_tahmin = st.button("😊", key="btn_check", type="primary")

# --- Enter Veya Buton Tetikleme Kontrolü ---
# Kullanıcı yazı yazıp Enter'a bastığında veya butona tıkladığında çalışır
is_submitted = btn_tahmin or (tahmin_input != "" and "prev_input" in st.session_state and st.session_state.prev_input != tahmin_input)
st.session_state.prev_input = tahmin_input
...
if st.session_state.last_feedback:
    if st.session_state.feedback_type == "warning":
        ...
        st.balloons()
        # Tebrikler/Kazanma Şarkısı Doğrudan Çalar
