...
if 'hedef_sayi' not in st.session_state:
    ...
    st.session_state.last_feedback = None  # Son uyarı mesajı hafızası
...
# --- Tahmin Kontrolü & INPUT'UN HEMEN ALTINDAKİ ALERT ALANI ---
if btn_tahmin:
    if tahmin_input.isdigit():
        ...
        if current_tahmin < st.session_state.hedef_sayi:
            ...
            st.session_state.last_feedback = f"Cıkk! Daha BÜYÜK bir sayı söylemelisin! ⬆️"
            ...
            st.session_state.last_feedback = f"Hımm! Daha KÜÇÜK bir sayı söylemelisin! ⬇️"
            st.session_state.feedback_type = "warning"
            st.rerun()
